...
-- | Apply a parallel list of 'POStags's to a 'Sentence'.
applyTags :: POStags t => Sentence -> [t] -> TaggedSentence t
applyTags (Sent ts) tags = TaggedSent $ zipWith POS tags ts

-- | A chunked sentence has POS tags and chunk tags. Generated by a
-- chunker.
--
-- (chunker :: (Chunk chunk, POStags tag) => TaggedSentence tag -> ChunkedSentence chunk tag)
...
showChunkedSent :: (ChunkTag c, POStags t) => ChunkedSentence c t -> Text
...
instance (ChunkTag c, Arbitrary c, Arbitrary t, POStags t) =>
...
-- tagger. (tagger :: POStags tag => Sentence -> TaggedSentence tag)
...
unzipTags :: POStags t => TaggedSentence t -> (Sentence, [t])
unzipTags (TaggedSent ts) =
  let (tags, toks) = unzip $ map topair ts
      topair (POS tag tok) = (tag, tok)
  in (Sent toks, tags)

unzipChunks :: (ChunkTag c, POStags t) => ChunkedSentence c t -> (TaggedSentence t, [c])
...
combine :: POStags t => [TaggedSentence t] -> [TaggedSentence t] -> [TaggedSentence t]
...
combineSentences :: POStags t => TaggedSentence t -> TaggedSentence t -> TaggedSentence t
...
pickTag :: POStags t => POS t -> POS t -> POS t
...
instance (ChunkTag c, Arbitrary c, Arbitrary t, POStags t) => Arbitrary (ChunkOr c t) where
...
mkChunk :: (ChunkTag chunk, POStags tag) => chunk -> [ChunkOr chunk tag] -> ChunkOr chunk tag
mkChunk chunk children = Chunk_CN (Chunk chunk children)

-- | Helper to create 'ChunkOr' types that just hold POS tagged data.
mkChink :: (ChunkTag chunk, POStags tag) => tag -> Token -> ChunkOr chunk tag
mkChink tag token      = POS_CN (POS tag token)


instance (ChunkTag c, Arbitrary c, Arbitrary t, POStags t) => Arbitrary (Chunk c t) where
...
instance (Arbitrary t, POStags t) => Arbitrary (POS t) where
  arbitrary = POS <$> arbitrary <*> arbitrary

-- | Show the underlying text token only.
showPOStok :: POStags tag => POS tag -> Text
showPOStok (POS _ (Token txt)) = txt

showPOStag :: POStags tag => POS tag -> Text
showPOStag = tagTerm . posTag

-- | Show the text and tag.
printPOS :: POStags tag => POS tag -> Text
...
contains :: POStags t => TaggedSentence t -> Text -> Bool
